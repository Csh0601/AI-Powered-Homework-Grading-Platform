...
class QwenService:
    def __init__(self, model_name: str = "qwen3:30B"):
        ...
        Args:
            model_name: Qwen 模型名称，默认为 qwen3:30B
        """
        ...
        if hasattr(Config, 'OLLAMA_BASE_URL'):
            ...
            self.client = ollama.Client(
                host=self.base_url,
                timeout=httpx.Timeout(60.0)  # 设置60秒超时
            )
            logger.info(f"配置远程Ollama服务器: {self.base_url}")
        else:
            # 使用本地服务器
            self.client = ollama
            logger.info("使用本地Ollama服务器")
    ...
    def _test_connection(self):
        ...
        try:
            ...
            # 测试模型生成功能（使用较短的prompt和少量tokens）
            logger.info(f"测试模型 {self.model_name} 生成功能...")
            test_response = self.client.generate(
                model=self.model_name,
                prompt="Hi",
                options={
                    'num_predict': 3,  # 只生成3个token用于测试
                    'temperature': 0.1
                },
                stream=False
            )
            ...
            if test_response and 'response' in test_response:
                ...
                raise Exception(f"模型 {self.model_name} 响应格式异常")
                
        except Exception as e:
            logger.error(f"❌ 连接 Ollama 服务失败: {e}")
            # 提供更详细的错误信息
            if "502" in str(e):
                logger.error("502错误：可能是Ollama服务未启动或端口未开放")
